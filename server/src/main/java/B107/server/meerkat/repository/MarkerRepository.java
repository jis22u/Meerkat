...
public interface MarkerRepository extends JpaRepository<Marker, Long> {
	...
	@Query("SELECT m.member.idx " +
			...
			"AND (m.member.idx <> :ownerId OR m.member.idx IS NULL)")
	List<Long> findValidMarkers(@Param("lat") Double lat, @Param("lng") Double lng, @Param("range") Double range, @Param("idx") Long ownerId);
